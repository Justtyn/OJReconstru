...
表：`homework`（作业基本信息）、`homework_problem`（作业题目关联）

### 查询

- `GET /api/homeworks`
  - 参数：`page`、`size`、`classId`、`activeOnly`
  - 角色：学生/教师/管理员登录可用；学生始终只能看到启用作业。
- `GET /api/homeworks/{id}`
  - 返回单个作业详情；未启用作业仅教师/管理员可见。
- `GET /api/homeworks/{id}/problems`
  - 返回该作业下的题目列表（直接关联 `problem` 表）。

### 管理（教师或管理员）

- `POST /api/homeworks`
  - body：`{ title, classId, startTime?, endTime?, description?, isActive?, problemIds:[1,2,...] }`
  - 创建时需同时传入题目 ID 列表；系统校验班级存在且（教师场景）为当前教师创建的班级。
- `PUT /api/homeworks/{id}`
  - body 同上，字段可选；如带 `problemIds` 则替换整个作业题目列表。
- `DELETE /api/homeworks/{id}`
- `POST /api/homeworks/{id}/problems`
  - body：`{ "problemIds":[...] }`，向作业追加题目，自动去重。
- `DELETE /api/homeworks/{id}/problems/{problemId}`
  - 从作业移除指定题目。

> 表关系：`homework.class_id -> classes.id`，`homework_problem.homework_id -> homework.id`，`homework_problem.problem_id -> problem.id`。删除作业会级联移除 `homework_problem`。
...
表：`discussion`（student_id、problem_id、title、content、is_active）、`discussion_comment`（discussion_id、student_id、content）

### 讨论主题

- `GET /api/discussions`
  - 参数：`page`、`size`、`problemId`、`userId`、`includeInactive(仅管理员)`
  - 学生/教师/管理员登录即可使用；学生只能看到 `is_active=1` 的讨论。
- `GET /api/discussions/{id}`
  - 详情，未启用讨论仅作者或管理员可看。
- `POST /api/discussions`
  - 仅学生可发，body：`{ title, content, problemId? }`。
- `PUT /api/discussions/{id}`
  - 作者或管理员可改。仅管理员能修改 `isActive`。
...
## 十一、提交 & 判题模块（Submission）

表：`submission`、`submission_testcase_result`、`submission_overall_status`、`judge_status`

### API

- `POST /api/submissions`
  - 仅学生可调用，body：`problemId`、`homeworkId?`、`languageId`、`sourceCode`
  - 逻辑：读取题目的所有 `problem_testcase`，逐个调用 Judge0（默认 `wait=true`），生成 `submission` 记录与多条 `submission_testcase_result`，根据 AC 数量写入整体状态（AC/部分错误/错误/系统错误），并计算分数（通过率 × 100）。
  - 校验：题目必须存在且启用；传入 `homeworkId` 时需确认此题已被作业关联。
- `GET /api/submissions`
  - 参数：`page`、`size`、`problemId`、`homeworkId`、`studentId`、`statusId`
  - 权限：学生仅能查看自己的提交；教师/管理员可按学号过滤。
- `GET /api/submissions/{id}`
  - 返回提交详情、源代码（仅作者或管理员可见）、以及所有测试用例的 Judge0 结果（stdout/stderr/status/time/memory）。

### 判题策略

1. 每个测试用例单独调用 `POST /submissions?base64_encoded=false&wait=true`，参数含 `source_code`、`language_id`、`stdin`、`expected_output`。
2. 保存 Judge0 返回的 `token/status/stdout/stderr/compile_output/message/time/memory`。
3. 汇总：
   - 全 AC → `submission.overall_status_id = 3 (ACCEPTED)`
   - 部分 AC → `4 (PARTIAL_WRONG)`
   - 全失败 → `5 (WRONG)`
   - 调用异常 → `6 (SYSTEM_ERROR)`
4. `submission.score = round(passed / total * 100)`，并记录 `passed_case_count / total_case_count`。
5. 后续可扩展到异步判题或队列（当前实现为同步逐个测试点）。

> Judge0 地址通过 `judge0.base-url` 配置（默认 `http://139.59.227.54:2358`）。测试环境使用 Stub，避免调用真实判题机。
