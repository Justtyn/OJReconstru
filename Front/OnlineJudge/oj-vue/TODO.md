...
## 留痕记录
- 2025-12-26：管理端加入路由过渡与加载态、背景动效装饰层，并套用 Markdown 渲染样式优化。
- 2025-12-26：修复客户端背景动效导致的页面底部/右侧空白溢出（限制溢出并隐藏横向滚动）。
- 2025-12-26：客户端加入路由过渡与加载态；新增背景动效装饰层；Markdown 渲染样式优化（代码块/行内代码/表格等）。
- 2025-12-26：个人中心“个人信息”列宽收窄，关于页项目信息侧栏加宽。
- 2025-12-26：题库与排行榜搜索区间距对齐题解/讨论风格（padding 与上下间距调整，筛选宽度一致）。
- 2025-12-26：提交记录学生列只显示姓名（客户端与管理端）。
- 2025-12-26：提交记录列表隐藏“提交ID”列（客户端与管理端）。
- 2025-12-26：提交记录列表补充默认单元格渲染，修复提交 ID 等字段不显示。
- 2025-12-26：客户端内容区加宽；导航栏角色胶囊高度调整；个人中心头像上传限制 1MB 并提示；学校枚举加载与缓存预取优化；背景区再增高。
- 2025-12-26：客户端内容区略微收窄；个人中心背景区增高、头像改为上传、邮箱禁改、学校改大学枚举；关于页三项介绍改为右侧横排；导航栏身份标识胶囊化。
